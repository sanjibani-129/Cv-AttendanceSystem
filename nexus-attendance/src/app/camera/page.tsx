...
// member for this long — stops the scan loop's next frame from immediately
...
export default function CameraPage() {
  ...
  // Always stop through this ref, never by reading video.srcObject — the
  ...
  const boot = useCallback(async () => {
    ...
    // dev-only mount→unmount→remount cycle fires) before getUserMedia
    ...
    // wants it — this is what previously left the camera light stuck on.
    let cancelled = false;
  ...
  const confirmCheckIn = useCallback(async () => {
    ...
    try {
      ...
      push(err instanceof Error ? err.message : "Couldn't log attendance — try again.", "error");
    } finally {
      setConfirming(false);
    }
        ...
        <div className="space-y-6">
          <div className="card p-5">
            ...
            >
              ...
              {confirming ? "Confirming…" : onCooldown ? "Logged — wait a moment" : "Confirm Check-in / Check-out"}
            </button>
          </div>
